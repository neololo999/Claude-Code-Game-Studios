...
## Mutable capture wrapper for lambda callbacks — avoids CONFUSABLE_CAPTURE_REASSIGNMENT
## lint warnings. Property access on a RefCounted is not flagged by the checker.
class _Cap extends RefCounted:
...
func _test_ac01_dig_started_emitted() -> void:
	...
	(world["dig"] as DigSystem).dig_started.connect(
		func(_c: int, _r: int) -> void: cap.fired = true
	)

	input.dig_requested.emit(Vector2i(-1, 0))

	var dig_state: TerrainSystem.DigState = terrain.get_dig_state(1, 3)
	var ok: bool = cap.fired and dig_state == TerrainSystem.DigState.DIGGING
	if ok:
		...
		print(
			...
			+ "got sig_fired=%s terrain=%d" % [cap.fired, dig_state]
		)
...
func _test_ac04_double_dig_second_rejected() -> void:
	...
	var cap := _Cap.new()
	(world["dig"] as DigSystem).dig_started.connect(
		func(_c: int, _r: int) -> void: cap.count += 1
	)
...
func _test_ac05_notify_digging_before_dig_request() -> void:
	...
	terrain.dig_state_changed.connect(
		func(_c: int, _r: int, _old: TerrainSystem.DigState, _new: TerrainSystem.DigState) -> void:
			cap.fired = gravity._digging_entities.has(0)
	)
	...
	if cap.fired:
		...
		print(
			"[FAIL] AC-05: notify_digging(true) was NOT in effect when terrain fired dig_state_changed — EC-02 ordering violated"
		)
...
func _test_ac06_after_cooldown_new_dig_accepted() -> void:
	...
	var cap := _Cap.new()
	dig.dig_started.connect(func(_c: int, _r: int) -> void: cap.count += 1)
...
func _test_ac09_dig_toward_open_rejected() -> void:
	...
	var cap := _Cap.new()
	dig.dig_started.connect(func(_c: int, _r: int) -> void: cap.fired = true)

	input.dig_requested.emit(Vector2i(-1, 0))

	var ok: bool = state_open == TerrainSystem.DigState.OPEN and not cap.fired
	if ok:
		...
		print(
			...
			% [state_open, TerrainSystem.DigState.OPEN, cap.fired]
		)
